class DigLibApp {
  ...
  async loadFeatured() {
    ...
    if (fileName === 'index.html' || !fileName || fileName.trim === '') {
      this.featuredDiv.innerHTML = this.featured.sort(() => Math.random() - 0.5).slice(0,4).sort((a, b) => parseFloat(a.item.originInfo_dateCreated) - parseFloat(b.item.originInfo_dateCreated)).map(this.toFeatureBlock).join('\n');
    } else {
      this.featuredDiv.innerHTML = this.featured.sort((a, b) => parseFloat(a.item.originInfo_dateCreated) - parseFloat(b.item.originInfo_dateCreated)).map(this.toFeatureBlock).join('\n');
    }
  }
  ...
  async loadTimeline() {
    ...
    if (fileName === 'index.html' || !fileName || fileName.trim === '') {
      this.timelineDiv.innerHTML = this.timeline.sort(() => Math.random() - 0.5).slice(0,4).sort((a, b) => parseFloat(a.item.originInfo_dateCreated) - parseFloat(b.item.originInfo_dateCreated)).map(this.toTimelineBlock).join('\n');
    } else {
      this.timelineDiv.innerHTML = this.timeline.sort((a, b) => parseFloat(a.item.originInfo_dateCreated) - parseFloat(b.item.originInfo_dateCreated)).map(this.toTimelineBlock).join('\n');
    }
  }
